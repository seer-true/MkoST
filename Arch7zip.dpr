...
//Callback('Ошибка создания канала');
  ...
  try
    ...
    CmdLine := Format('"%s" a -tzip "%s" "%s\*"', [SevenZipPath, string(ArchiveName), string(FolderPath)]);
    FillChar(StartupInfo, SizeOf(TStartupInfo), 0);
    StartupInfo.cb := SizeOf(TStartupInfo);
    StartupInfo.dwFlags := STARTF_USESHOWWINDOW or STARTF_USESTDHANDLES;
    StartupInfo.wShowWindow := SW_HIDE;
    ...
    begin
      try
        CloseHandle(hWritePipe); //Закрываем записывающий конец в основном процессе
        Callback(PChar('Начато архивирование: ' + string(FolderPath)));
        //Читаем вывод процесса
        while True do
        begin
          if not PeekNamedPipe(hReadPipe, nil, 0, nil, @BytesRead, nil) or (BytesRead = 0) then
          begin
            if WaitForSingleObject(ProcessInfo.hProcess, 100) <> WAIT_TIMEOUT then
              Break;
          end
          ...
          begin
            ReadFile(hReadPipe, Buffer, SizeOf(Buffer) - 1, BytesRead, nil);
            if BytesRead > 0 then
            begin
              Buffer[BytesRead] := #0;
              Output := string(AnsiString(Buffer));
